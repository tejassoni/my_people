$(document).ready(function() {
    ...
    $('#eye_brow_color').colorpicker();

    /* File Upload Starts  */
    $("#eye_brow_img").on("change", function() {
    ...
    $('#eyebrow_list_table').DataTable({
        ...
        columns: [{ // First Column As a Checkbox
            ...
            render: function(data, type, full, meta, row) { // function to modify dynamic data
                return '<input type="checkbox" class="child_chkbox" name="child_chkbox[]" value="' + $('<div/>').text(full.eye_brow_id).html() + '">';
            }
        }, {
            width: "10%",
            ...
            data: 'eye_brow_description',
            name: 'eye_brow_description',
            title: "Description",
            orderable: true,
            searchable: true
        }, {
    ...
    function ajaxDelete(ids = []) {
        $.ajax({
            url: APPURL + "/admin/delete_eyebrow",
            type: "POST",
            data: { 'ids': ids },
            dataType: 'JSON',
            beforeSend: function() {
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.status) {
                    ...
                    success_body += 'Eyebrows are Deleted successfully.';
                    ...
                    var warning_body = "";
                    warning_head += '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Sorry, Operation Fails...!';
                    warning_body += 'Eyebrows are not deleted... Please try after sometime. ';
                    $(".modal-header h4").html(warning_head);
                    $(".modal-body p").html(warning_body);
                    $('.error_modal').trigger('click');
                    setTimeout(function() { location.reload(); }, 2000);
    ...
    function ajaxDeleteSpecific(ids) {
        $.ajax({
            url: APPURL + "/admin/eyebrow_delete/" + ids,
            type: "get",
            dataType: 'JSON',
            beforeSend: function() {
                $(".modal-header h4").html("");
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.status) {
                    ...
                    var success_body = "";
                    success_head += '<i class="fa fa-check-circle" aria-hidden="true"></i> Success..!';
                    success_body += 'Eyebrow is Deleted successfully.';
                    $(".modal-header h4").html(success_head);
                    $(".modal-body p").html(success_body);
                    $('.error_modal').trigger('click');
                    setTimeout(function() { location.reload(); }, 2000);
                    ...
                    var warning_body = "";
                    warning_head += '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Sorry, Operation Fails...!';
                    warning_body += 'Eyebrow is not deleted... Please try after sometime. ';
                    $(".modal-header h4").html(warning_head);
                    $(".modal-body p").html(warning_body);
                    $('.error_modal').trigger('click');
                    setTimeout(function() { location.reload(); }, 2000);
    ...
    $(document).on('click', '.btn_status', function() {
        ...
        $.ajax({
            url: APPURL + "/admin/eyebrow_status",
            type: "POST",
