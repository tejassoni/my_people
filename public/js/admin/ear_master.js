$(document).ready(function() {
    ...
    $('#ear_color').colorpicker();

    /* File Upload Starts  */
    $("#ear_img").on("change", function() {
    ...
    $('#ear_list_table').DataTable({
        ...
        columns: [{ // First Column As a Checkbox
            ...
            render: function(data, type, full, meta, row) { // function to modify dynamic data
                return '<input type="checkbox" class="child_chkbox" name="child_chkbox[]" value="' + $('<div/>').text(full.ear_id).html() + '">';
            }
        }, {
            width: "10%",
            ...
            data: 'ear_description',
            name: 'ear_description',
            title: "Description",
            orderable: true,
            searchable: true
        }, {
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.status) {
                    ...
                    success_body += 'Ears are Deleted successfully.';
                    ...
                    var warning_body = "";
                    warning_head += '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Sorry, Operation Fails...!';
                    warning_body += 'Ears are not deleted... Please try after sometime. ';
                    $(".modal-header h4").html(warning_head);
                    $(".modal-body p").html(warning_body);
                    $('.error_modal').trigger('click');
                    setTimeout(function() { location.reload(); }, 2000);
    ...
    function ajaxDeleteSpecific(ids) {
        $.ajax({
            url: APPURL + "/admin/ear_delete/" + ids,
            type: "get",
            dataType: 'JSON',
            beforeSend: function() {
                $(".modal-header h4").html("");
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.status) {
                    ...
                    var success_body = "";
                    success_head += '<i class="fa fa-check-circle" aria-hidden="true"></i> Success..!';
                    success_body += 'Ear is Deleted successfully.';
                    $(".modal-header h4").html(success_head);
                    $(".modal-body p").html(success_body);
                    $('.error_modal').trigger('click');
                    setTimeout(function() { location.reload(); }, 2000);
                    ...
                    var warning_body = "";
                    warning_head += '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Sorry, Operation Fails...!';
                    warning_body += 'Ear is not deleted... Please try after sometime. ';
                    $(".modal-header h4").html(warning_head);
                    $(".modal-body p").html(warning_body);
                    $('.error_modal').trigger('click');
                    setTimeout(function() { location.reload(); }, 2000);
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                if (data.status) {
                    ...
                    var warning_body = "";
                    warning_head += '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Sorry, Operation Fails...!';
                    warning_body += 'Ear is In Activated... Please try after sometime. ';
                    $(".modal-header h4").html(warning_head);
                    $(".modal-body p").html(warning_body);
                    $('.error_modal').trigger('click');
                    setTimeout(function() { location.reload(); }, 2000);
